...
void config_tune_drive()
{
	...
	std::function<float(double)> actual_plot = [](double x)
	{ return chassis.get_ForwardTracker_position(); };
	...
	if (testing_odom)
	{
		actual_plot = [](double x)
		{ return hypot(chassis.get_X_position(), chassis.get_Y_position()); };
		set_point_plot = [](double x)
		{ return hypot(chassis.desired_X_position, chassis.desired_Y_position); };
	}
...
void config_tune_heading()
{
	...
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({[](double x)
						 { return chassis.inertial.rotation(); },
						 [](double x)
						 {
...
void config_tune_turn()
{
	...
	graph_scr->set_plot_bounds(y_min, y_max, 0, time_spent_graphing_ms, 1, 1);
	graph_scr->set_plot({[](double x)
						 { return chassis.get_absolute_heading(); },
						 [](double x)
						 { return reduce_0_to_360(chassis.desired_angle); }},
						{{"Actual", 0x002E8B59},
						 {"SetPoint", 0x00FA8072}});
	UI_select_scr(graph_scr->get_graph_screen());
...
void config_odom_data()
{
	...
	console_scr->add("X: ", []()
					 { return chassis.get_X_position(); });
	console_scr->add("Y: ", []()
					 { return chassis.get_Y_position(); });
	console_scr->add("Heading: ", []()
					 { return chassis.get_absolute_heading(); });
	console_scr->add("Rotation: ", []()
					 { return chassis.inertial.rotation(); });
	console_scr->add("Forward_Tracker: ", []()
					 { return chassis.get_ForwardTracker_position(); });
	console_scr->add("Sideways_Tracker: ", []()
					 { return chassis.get_SidewaysTracker_position(); });
